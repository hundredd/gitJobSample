...
//        NSInteger videoMemerySize = tempData.length / (1024 * 1024);
        ...
        //临时写死
        NSInteger videoMemerySize = 300;
        // 如果视频大于40M 则不允许上传
        if (videoMemerySize > 200) {
            
//            NSString *path = [[NSBundle mainBundle] pathForResource:@"3分钟.MOV" ofType:nil];
            NSString *path = [[NSBundle mainBundle] pathForResource:@"独白.mp4" ofType:nil];
            
            [TranTool convertVideoWithModel:path
                           andProccessBlock:nil
                             andFinishBlock:nil
                                   andError:nil];
            // 提示 @"视频大于200M, 无法上传"
        } else {
                ...
                [upLoadMeneger postImageWithURLString:@"上传图片的连接"  parameter:@{@"传图的参数":@""} image:weakSelf.videoImag.image success:^(UpLoadImage *net, id object) {
                    ...
                    if ([object[@"result"] integerValue] == 1) {
                        ...
                        if (videoMemerySize <= 8) {
                            // 视频小于8M  直接上传
                            // 上传视频
                            [upLoadMeneger uploadFileWithMediaData:nil url:PostVideoURL params:@{@"上传视频参数":@""}];
