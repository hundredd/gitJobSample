...
- (void)drawLine{
    if (self.dataArray||self.dataRightArray)//分别绘制左边线和右边线条
    {
        ...
        if (self.leftAvg) {
            [self singleLeftAvgLine];
        }
        if (self.rightAvg) {
            [self singleRightAvgLine];
        }
    }
...
#pragma mark  //左边平均线
-(void)singleLeftAvgLine
{
    CGFloat maxValue = [self arrMaxValue:[self dataArrFromModelArr:self.dataArray]];
    //这是间隔的高度,主要是顶部的要去掉
    ...
    CGFloat rate = (maxValue - self.leftAvg.floatValue) / maxValue;
    //注意0点处
    //totalH*rate -YCommonMargin;//如果最高点算  最高点不算 YCommonMargin+sperateH
    CGFloat y = lineTotalH*rate - YCommonMargin+sperateH;
    
    [path moveToPoint:CGPointMake(XCommonMargin, y)];
    [path addLineToPoint:CGPointMake(centerX - bounceX,y)];
    CAShapeLayer * dashLayer = [CAShapeLayer layer];
    dashLayer.strokeColor = ChartLeftColor.CGColor;
    dashLayer.fillColor = [UIColor clearColor].CGColor;
    dashLayer.lineWidth = 0.6;
    dashLayer.path = path.CGPath;
    NSArray *dash = @[@3,@2];
...
#pragma mark  //右边平均线
-(void)singleRightAvgLine
{
    CGFloat maxValue = [self arrMaxValue:[self dataArrFromModelArr:self.dataRightArray]];
    //这是间隔的高度,主要是顶部的要去掉
    CGFloat marginY = bounceY*ChartYRate;
    CGFloat totalH = self.frame.size.height - marginY - bounceY +YCommonMargin;
    CGFloat sperateH = (totalH)/self.rowCount;
    ...
    //这里要注意,最高点的值是不要的,所以应该剪掉
    CGFloat rate = (maxValue - self.rightAvg.floatValue) / maxValue;
    //注意0点处
    //totalH*rate -YCommonMargin;//如果最高点算  最高点不算 YCommonMargin+sperateH
    CGFloat y = lineTotalH*rate - YCommonMargin+sperateH;
    ...
    [path addLineToPoint:CGPointMake(centerX - bounceX,y)];
    CAShapeLayer * dashLayer = [CAShapeLayer layer];
    dashLayer.strokeColor = ChartRightColor.CGColor;
    dashLayer.fillColor = [UIColor clearColor].CGColor;
    dashLayer.lineWidth = 0.6;
    dashLayer.path = path.CGPath;
    NSArray *dash = @[@3,@2];
...
-(void)showDetailLineStyle
{
    ...
    [self showDetailShowWithTitle:@"真的" andIndex:0 andColor:ChartLeftColor];
    
}

-(void)showDetailShowWithTitle:(NSString *)title andIndex:(NSInteger)index andColor:(UIColor *)color
{
    CGFloat x = 10;
    CGFloat marginY = 10;
    CGFloat y = (self.frame.size.height -10) - marginY*index;
    
    CGFloat width = 30;
    
    CGFloat lbx = x+width + XCommonMargin;
    ...
    UILabel * showLb = [UILabel new];
    showLb.textColor = color;
    showLb.center = (CGPoint){lbx,y};
    showLb.font = [UIFont systemFontOfSize:10];
    showLb.text = title;
    [showLb sizeToFit];
    [self addSubview:showLb];
}

#pragma mark //绘画以左边为基准的折线
-(void)singleLeftLine
{
    //创建左边的label数目
...
-(void)setYLeftTitle:(NSString *)yLeftTitle
{
    ...
    [label sizeToFit];
    [self addSubview:label];
    label.center = (CGPoint){15,self.frame.size.height/2.0};
...
-(void)setYRightTitle:(NSString *)yRightTitle
{
    ...
    [label sizeToFit];
    [self addSubview:label];
    label.center = (CGPoint){self.frame.size.width - 20,self.frame.size.height/2.0};
}
...
-(void)setTopTitle:(NSString *)topTitle
{
    ...
    [label sizeToFit];
    [self addSubview:label];
    label.center = (CGPoint){self.frame.size.width /2.0 ,10};
}
